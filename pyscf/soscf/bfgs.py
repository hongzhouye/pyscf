...
def rotate_orb_cc(
    ...
    """
    Container-agnostic L-BFGS rotation iterator.
    Works for molecular and k-point provided the localizer defines:
      - get_grad(u)->packed ndarray
      - cost_function(u)->scalar (maximize)
      - extract_rotation(dr)->u_step
      - update_rotation(u, u_step)->u_new
    Optional:
      - eye_rotation(u)->identity step object
    """
    ...
    class Statistic:
        def __init__(self):
            self.tot_kf = 0   # gradient builds (PySCF naming)
