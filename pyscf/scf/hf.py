...
def kernel_hy(mf, conv_tol=1e-10, conv_tol_grad=None,
           dump_chk=True, dm0=None, callback=None, conv_check=True, **kwargs):
    '''kernel: the SCF driver.
...
Keyword argument "init_dm" is replaced by "dm0"''')
    ...
    # Skip SCF iterations. Compute only the total energy of the initial density
    if mf.max_cycle <= 0:
        fock = mf.get_fock_hy(h1e, s1e, vhf, dm)  # = h1e + vhf, no DIIS
        mo_energy, mo_coeff = mf.eig(fock, s1e)
        mo_occ = mf.get_occ(mo_energy, mo_coeff)
        return scf_conv, e_tot, mo_energy, mo_coeff, mo_occ
    ...
    if isinstance(mf.diis, lib.diis.DIIS):
        ...
        # Since the ingredients for the Fock matrix has already been built, we can
        # just go ahead and use it to determine the orthonormal basis vectors.
        fock = mf.get_fock_hy(h1e, s1e, vhf, dm)
        _, Corth = mf.eig(fock, s1e)
        mf_diis = mf.DIIS(mf, Corth)
        mf_diis.space = mf.diis_space
    ...
    for cycle in range(mf.max_cycle):
        ...
        if use_level_shift:
            use_level_shift = delta_E is None or abs(delta_E) > mf.level_shift_conv_tol
        level_shift_factor = mf.level_shift if use_level_shift else 0
        fock = mf.get_fock_hy(h1e, s1e, vhf, dm, cycle, mf_diis,
                           level_shift_factor=level_shift_factor, fock_last=fock_last)
        fock_last = fock
        mo_energy, mo_coeff = mf.eig(fock, s1e)
        mo_occ = mf.get_occ(mo_energy, mo_coeff)
...
def get_fock_hy(mf, h1e=None, s1e=None, vhf=None, dm=None, cycle=-1, diis=None,
    ...
    if 0 <= cycle < diis_start_cycle-1 and abs(damp_factor) > 1e-4:
        ...
        if fock_last is not None:
            ...
            f += (fock_last - f) * damp_factor
    if diis is not None and cycle >= diis_start_cycle:
        f = diis.update(s1e, dm, f, fock_last)
    if abs(level_shift_factor) > 1e-4:
        f = level_shift(s1e, dm*.5, f, level_shift_factor)
    return f
...
employing the updated GWH rule from doi:10.1021/ja00480a005.''')
    ...
    def scf_hy(self, dm0=None, **kwargs):
        '''SCF main driver
        ...
        if self.max_cycle > 0 or self.mo_coeff is None:
            self.converged, self.e_tot, \
                    self.mo_energy, self.mo_coeff, self.mo_occ = \
                    kernel_hy(self, self.conv_tol, self.conv_tol_grad,
                           dm0=dm0, callback=self.callback,
                           conv_check=self.conv_check, **kwargs)
        else:
            # Avoid to update SCF orbitals in the non-SCF initialization
            # (issue #495).  But run regular SCF for initial guess if SCF was
            # not initialized.
            self.e_tot = kernel_hy(self, self.conv_tol, self.conv_tol_grad,
                                dm0=dm0, callback=self.callback,
                                conv_check=self.conv_check, **kwargs)[1]
        ...
        self._finalize()
        return self.e_tot
    kernel_hy = lib.alias(scf_hy, alias_name='kernel_hy')
