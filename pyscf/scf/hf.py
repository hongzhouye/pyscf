...
def kernel(mf, conv_tol=1e-10, conv_tol_grad=None,
           dump_chk=True, dm0=None, mo_coeff0=None, callback=None,
    ...
    Kwargs:
        ...
        mo_coeff0 : ndarray
            Initial guess mo coeff matrix.  It's only effective when ``mf.mom`` is set True, in which case a ``dm`` is computed from mo_coeff0 and overwrites the input ``dm0``.
...
Keyword argument "init_dm" is replaced by "dm0"''')
    ...
    # if mom is requested:
    #   1. input "mo_coeff0" is required, whose occupancy can be adjusted by "mf.orb_swap"
    #   2. input "dm0" is over-written by rdm1 derived from mo_coeff0
    if mf.mom:
        dm0 = mf.get_mom_guess(mo_coeff0)
    ...
    if mf.max_cycle <= 0:
        ...
        if mf.mom:
            mo_occ = mf.get_occ(mo_energy, mo_coeff, dmref=dm, s1e=s1e)
        else:
...
def get_occ(mf, mo_energy=None, mo_coeff=None, dmref=None, s1e=None):
    ...
    Kwargs:
        ...
        dmref : 2D ndarray
            Reference rdm1 for MOM to determine occupancy
