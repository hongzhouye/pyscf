...
def get_occ(mf, mo_energy=None, mo_coeff=None, dmref=None, s1e=None):
    ...
    if mf.verbose >= logger.INFO and n_a < nmo and n_b > 0 and n_b < nmo:
        if e_sort_a[n_a-1]+1e-3 > e_sort_a[n_a] and not mf.mom:
        ...
        if e_sort_b[n_b-1]+1e-3 > e_sort_b[n_b] and not mf.mom:
        ...
        if e_sort_a[n_a-1]+1e-3 > e_sort_b[n_b] and not mf.mom:
