...
[ ] make C code compute j3c in (L|ij) order
[ ] calculate needed ovlp shls_slice only in add_j3c_sr_G0_q_
...
def aux_e2_nospltbas(cell, auxcell_or_auxbasis, omega, intor='int3c2e', aosym='s2ij',
                     comp=None, kptij_lst=None, shls_slice=None, bvk_kmesh=None,
                     precision=None, estimator="ME", verbose=None, out=None):
    ...
    **This function should be only used by RSGDF**
    ...
    int3c = wrap_int3c_nospltbas(cell, auxcell, omega, shlpr_mask,
                                 ...
                                 kptij_lst, bvk_kmesh=bvk_kmesh, order='Lij',
                                 verbose=verbose)
...
def get_j3c_sr(mydf, cell=None, auxcell=None, kptij_lst=None, shls_slice=None,
               ...
               estimator='ME', verbose=None, out=None):
    ...
    # TODO: make the C code calculate j3c in (L|ij) form directly to avoid copy!
    # out.shape = (nkptij, comp, naoaux, nao_pair)
    # out = np.asarray(out.transpose(0,1,3,2), order='C')
# add G=0 correction (this modifies out in situ)
    return out
...
def get_j3c(mydf, cell=None, auxcell=None, kptij_lst=None, shls_slice=None,
            ...
            precision=None, mesh=None, estimator='ME', exxdiv=None, out=None,
            verbose=None):
    if cell is None: cell = mydf.cell
    if auxcell is None: auxcell = mydf.auxcell
