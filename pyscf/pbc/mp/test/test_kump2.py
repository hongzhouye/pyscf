...
class Diamond_GDF(unittest.TestCase):
    ''' Diamond 211 kpt sampling with total spin = 2
        The number of occupied bands is [5,4] for spin up and [3,4] for spin down.
...
class Diamond_FFTDF(unittest.TestCase):
    ...
    def test_energy(self):
        mmp = mp.KMP2(self.mf).run()
        self.assertAlmostEqual(mmp.e_corr, -0.159839676890340, 6)

    def test_energy_frozen(self):
        mmp = mp.KMP2(self.mf, frozen=1).run()
        self.assertAlmostEqual(mmp.e_corr, -0.124626982939688, 6)
